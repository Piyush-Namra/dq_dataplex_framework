...
def process_excel_file(bucket_path, excel_file_name):
    ...
    dq_rule_expectation_dict = {
        'NOT_NULL': "non_null_expectation",
        'RANGE': "range_expectation",
        'UNIQUE': "uniqueness_expectation",
        'SQL_ROW': "row_condition_expectation",
        'REGEX': "regex_expectation",
        'SET': "set_expectation",
        'SQL_TABLE': "table_condition_expectation",
        'SQL_ASSERT': "sql_assertion"
    }
    ...
    for group_key, group_df in grouped:
        ...
        dq_export_notification_email_ids = table_level_row['DQ Export Notification Email IDs'].values[
            0] if not table_level_row.empty else '' 

        score_threshold = table_level_row['Score Threshold'].values[
            0] if not table_level_row.empty else '' 
        ...
        for _, row in group_df.iterrows():
            ...
            if group_key_str not in yaml_content_dict:
                yaml_content_dict[group_key_str] = {
                    ...
                    "dq_export_notification_email_ids": dq_export_notification_email_ids,
...
def write_yaml_files(yaml_content_dict, bucket_path):
    ...
    for key, value in yaml_content_dict.items():
        ...
        if value.get('schedule_interval') and isinstance(value['schedule_interval'], str) and value[
            ...
            yaml_file_name = key.replace('~', '__') + '__' + modified_schedule + '.json'
        else:
            yaml_file_name = key.replace('~', '__') + '.json'
        ...
        if value.get('partition_filter_condition'):
            yaml_content['row_filter'] = value['partition_filter_condition']

        # Add samplingPercent if data_sampling_percentage is not empty
        if value.get('data_sampling_percentage'):
            yaml_content['sampling_percent'] = float(value['data_sampling_percentage'])
        ...
        for i in range(len(value["expectation"])):
            ...
            if value["ignoreNull"][i]:
                rule["ignore_null"] = value["ignoreNull"][i].lower() == 'true'

            if value["minValue"][i]:
                # rule["minValue"] = value["minValue"][i]
                rule[value["expectation"][i]]["min_value"] = value["minValue"][i]
            if value["maxValue"][i]:
                # rule["maxValue"] = value["maxValue"][i]
                rule[value["expectation"][i]]["max_value"] = value["maxValue"][i]
            if value["strictMinEnabled"][i]:
                # rule["strictMinEnabled"] = value["strictMinEnabled"][i].lower() == 'true'
                rule[value["expectation"][i]]["strict_min_enabled"] = value["strictMinEnabled"][i].lower() == 'true'
                
            if value["strictMaxEnabled"][i]:
                # rule["strictMaxEnabled"] = value["strictMaxEnabled"][i].lower() == 'true'
                rule[value["expectation"][i]]["strict_max_enabled"] = value["strictMaxEnabled"][i].lower() == 'true'

            if value["sqlExpression"][i]:
                # rule["sqlExpression"] = value["sqlExpression"][i]
                if value["expectation"][i] == "sql_assertion":
                    rule[value["expectation"][i]]["sql_statement"] = value["sqlExpression"][i]
                else:
                    rule[value["expectation"][i]]["sql_expression"] = value["sqlExpression"][i]
        ...
        if value.get('dq_export_gcp_project_id') and isinstance(value['dq_export_gcp_project_id'], str) and value[
            ...
            if 'post_scan_actions' not in yaml_content:
                yaml_content['post_scan_actions'] = {}

            if 'bigquery_export' not in yaml_content['post_scan_actions']:
                yaml_content['post_scan_actions']['bigquery_export'] = {}

            yaml_content['post_scan_actions']['bigquery_export']['results_table'] = (
        ...
        if value.get('dq_export_notification_email_ids') and isinstance(value['dq_export_notification_email_ids'], str) and value[
            'dq_export_notification_email_ids'].strip():

            if 'post_scan_actions' not in yaml_content:
                yaml_content['post_scan_actions'] = {}

            if 'notification_report' not in yaml_content['post_scan_actions']:
                yaml_content['post_scan_actions']['notification_report'] = {}

            if 'recipients' not in yaml_content['post_scan_actions']['notification_report']:
                yaml_content['post_scan_actions']['notification_report']['recipients'] = {}

            if 'score_threshold_trigger' not in yaml_content['post_scan_actions']['notification_report']:
                yaml_content['post_scan_actions']['notification_report']['score_threshold_trigger'] = {}

            if 'job_failure_trigger' not in yaml_content['post_scan_actions']['notification_report']:
                yaml_content['post_scan_actions']['notification_report']['job_failure_trigger'] = {}

            if 'job_end_trigger' not in yaml_content['post_scan_actions']['notification_report']:
                yaml_content['post_scan_actions']['notification_report']['job_end_trigger'] = {}

            yaml_content['post_scan_actions']['notification_report']['recipients']['emails'] = value.get('dq_export_notification_email_ids').split(",")

            yaml_content['post_scan_actions']['notification_report']['score_threshold_trigger']['score_threshold'] = float(value.get('score_threshold'))
        else:
            pass
        ...
        try:
            ...
            blob.upload_from_string(yaml_str, content_type='application/json')
...
def main():
    ...
    """
    location = "gs://dataplex_demo_env/dq_excel/"
...
with DAG('dag_2_generate_json_config', default_args=default_args, schedule_interval=None,tags=['dataplex', 'gcp', 'dq', 'demo'],) as dag:
    test_task = PythonOperator(
        task_id='dag_2_generate_json_config',
